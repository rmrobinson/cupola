(function () {
  ...
  // SVG paths extracted directly from tar1090 markers.js
  // Each shape: { path, viewBox, sw (stroke-width in viewBox units) }
  const _AC_SHAPES = {
    airliner: {
      viewBox: '-1 -2 34 34', sw: 1.2,
    ...
    heavy_2e: {
      viewBox: '0 -3.2 64.2 64.2', sw: 1.8,
  ...
  function aircraftIcon(a) {
    const shapeName = _AC_CATEGORY_SHAPE[a.category] || 'airliner';
    const shape = _AC_SHAPES[shapeName];
    const angle = a.track ?? 0;
    const fill  = a.on_ground ? '#95a5a6' : '#4ade80';
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${SIZE}" height="${SIZE}" viewBox="${shape.viewBox}" style="transform:rotate(${angle}deg);transform-origin:center"><path d="${shape.path}" fill="${fill}" stroke="#1e293b" stroke-width="${shape.sw}"/></svg>`;
    return L.divIcon({ html: svg, className: 'aircraft-svg-icon', iconSize: [SIZE, SIZE], iconAnchor: [SIZE/2, SIZE/2], popupAnchor: [0, -SIZE/2] });
  }
